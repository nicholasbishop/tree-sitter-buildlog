LANGUAGE_NAME := tree-sitter-buildlog
HOMEPAGE_URL := https://github.com/nicholasbishop/tree-sitter-buildlog
VERSION := 0.1.0
DESCRIPTION := Tree-sitter grammar for parsing build logs
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/buildlog '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/buildlog
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/buildlog
